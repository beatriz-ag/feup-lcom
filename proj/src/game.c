...
void set_display() {
  ...
  loading = (struct sprite **) malloc(7 * sizeof(struct sprite));
  if (loading == NULL) {
    free(loading);
    return;
  }
  ...
  loading[2] = get_sprite((xpm_map_t) init2, 0, 0);
  loading[3] = get_sprite((xpm_map_t) init3, 0, 0);
  loading[4] = get_sprite((xpm_map_t) init4, 0, 0);
  loading[5] = get_sprite((xpm_map_t) init5, 0, 0);
  loading[6] = get_sprite((xpm_map_t) init6, 0, 0);

  display_xpm(main_buf, loading[0]->data, loading[0]->pos_x, loading[0]->pos_y, loading[0]->width, loading[0]->height);
}
...
void load_xpms() {
  ...
  display_xpm(main_buf, loading[3]->data, loading[3]->pos_x, loading[3]->pos_y, loading[3]->width, loading[3]->height);
...
void set_platforms() {
  ...
  platforms[1].pos_x = 850;
  platforms[1].pos_y = 755;
  platforms[1].width = 302;

  platforms[2].pos_x = 554;
  platforms[2].pos_y = 624;
  platforms[2].width = 300;

  platforms[3].pos_x = 378;
  platforms[3].pos_y = 507;
  platforms[3].width = 52;

  platforms[4].pos_x = 58;
  platforms[4].pos_y = 380;
  platforms[4].width = 306;

  platforms[5].pos_x = 750;
  platforms[5].pos_y = 408;
  platforms[5].width = 257;

  platforms[6].pos_x = 410;
  platforms[6].pos_y = 205;
  platforms[6].width = 52;

  platforms[8].pos_x = 500;
  platforms[8].pos_y = 160;
  platforms[8].width = 260;

  platforms[7].pos_x = 900;
  platforms[7].pos_y = 212;
  platforms[7].width = 150;
}

void set_up() {

  mouse_enable_data_report();
  rtc_enable_ints(RTC_REG_B_UI);
  ...
  display_xpm(main_buf, loading[1]->data, loading[1]->pos_x, loading[1]->pos_y, loading[1]->width, loading[1]->height);
  ...
  display_xpm(main_buf, loading[2]->data, loading[2]->pos_x, loading[2]->pos_y, loading[2]->width, loading[2]->height);
  load_xpms();
  set_buttons();
  tickdelay(micros_to_ticks(200000));
  display_xpm(main_buf, loading[4]->data, loading[4]->pos_x, loading[4]->pos_y, loading[4]->width, loading[4]->height);
  set_presents();
  tickdelay(micros_to_ticks(200000));
  display_xpm(main_buf, loading[5]->data, loading[5]->pos_x, loading[5]->pos_y, loading[5]->width, loading[5]->height);
  set_platforms();
  tickdelay(micros_to_ticks(200000));
  display_xpm(main_buf, loading[6]->data, loading[6]->pos_x, loading[6]->pos_y, loading[6]->width, loading[6]->height);
  tickdelay(micros_to_ticks(200000));
}
      ...
      switch (_ENDPOINT_P(msg.m_source)) {
        case HARDWARE:
          ...
          if (msg.m_notify.interrupts & rtc_irq_set) {
            rtc_ih(RTC_REG_B_UI);
            printf("%x:%x:%x      %x/%x/%x\n\n", curr_time.hours, curr_time.minutes, curr_time.seconds, curr_date.day, curr_date.month, curr_date.year);
          }
          break;
      }
    }
      ...
      switch (_ENDPOINT_P(msg.m_source)) {
        case HARDWARE:
          ...
          if (msg.m_notify.interrupts & timer_irq_set) {
            ...
            if ((timer_counter % HALF_SECOND_TICKS) == 0 && moving) {
              presents_rot++;
            }
            ...
            if (moving) { /*the countdown will start only once santa starts actually moving*/
              timer_int_handler();
              timer_events_handler();
            }
